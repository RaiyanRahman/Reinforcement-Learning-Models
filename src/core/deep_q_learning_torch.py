...
class DQN(QN):
    ...
    def build(self):
        ...
        if hasattr(self.config, 'load_path'):
            ...
            self.update_target()
            # Load DDDQN V and A layers.
            if self.dddqn:
                A_load_path = Path(self.config.A_load_path)
                V_load_path = Path(self.config.V_load_path)
                assert A_load_path.is_file(), f'Provided load_path ({A_load_path}) does not exist'
                assert V_load_path.is_file(), f'Provided load_path ({V_load_path}) does not exist'
                self.A.load_state_dict(torch.load(A_load_path, map_location='cpu'))
                self.V.load_state_dict(torch.load(V_load_path, map_location='cpu'))
    ...
    def save(self):
        ...
        # Save the A and V layers.
        if self.dddqn:
            torch.save(self.V.state_dict(), self.config.V_output)
            torch.save(self.A.state_dict(), self.config.A_output)
