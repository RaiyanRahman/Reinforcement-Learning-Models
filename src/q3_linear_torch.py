...
class Linear(DQN):
    ...
    def calc_loss(self, q_values : Tensor, target_q_values : Tensor,
        ...
        # Calculate Q_samp.
        Q_samp = rewards + ((1 - done_mask) * (gamma * torch.max(target_q_values, dim=1)[0]))
        # Extract Q(a) for the actions that were taken.
        Q = torch.sum(F.one_hot(actions, num_classes=num_actions) * q_values, dim=1)
        # Calculate the MSE loss.
        loss = F.mse_loss(Q, Q_samp)
